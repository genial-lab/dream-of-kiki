...
git commit -m "test(conformance): add DR-0 and DR-1 property tests"
```

Subject : 50 chars (EXACT boundary — verify). If 51, trim to `test(conformance): DR-0 and DR-1 property tests` (47).
...
- S5.3 subject `test(conformance): add DR-0 and DR-1 property tests` = 51 chars ❌ (51 > 50) → trim to `test(conformance): DR-0 and DR-1 property tests` = 47 chars ✅
...
- S6.2 subject `docs(proof): add G3-draft circulation log` = 41 chars (MODIFIED from 43 above — recount) → OK
