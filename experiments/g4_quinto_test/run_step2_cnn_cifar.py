"""G4-quinto Step 2 driver — H5-B test with CNN-on-CIFAR substrate.

Layout : 4 arms x N seeds x 1 HP (C5 anchor) on
``G4SmallCNN`` (latent_dim = 64). Mirrors Step 1 except that the
substrate consumes NHWC inputs directly and the beta buffer
stores ``(32, 32, 3)`` float32 records (the CNN's
``replay_optimizer_step`` accepts both flat-3072 and NHWC).

Outputs :
    docs/milestones/g4-quinto-step2-2026-05-03.{json,md}

Pre-reg : docs/osf-prereg-g4-quinto-pilot.md sec 2 (H5-B).
"""
...
DEFAULT_OUT_MD = (
    REPO_ROOT / "docs" / "milestones" / "g4-quinto-step2-2026-05-03.md"
)
DEFAULT_REGISTRY_DB = REPO_ROOT / ".run_registry.sqlite"
SMOKE_OUT_JSON = Path("/tmp") / "g4-quinto-step2-smoke.json"
SMOKE_OUT_MD = Path("/tmp") / "g4-quinto-step2-smoke.md"
RETENTION_EPS = 1e-6
...
class _BetaRecordCNN(TypedDict):
    """One CNN beta record : NHWC float32 array + label + latent."""

    x: list[list[list[float]]]  # serialised (32, 32, 3)
    y: int
...
def _dream_episode_cnn(
    ...
    *,
    ...
    """Drive one DreamEpisode and mutate CNN weights.
    ...
    episode = DreamEpisode(
        ...
        episode_id=f"g4-quinto-step2-{type(profile).__name__}-seed{seed}",
    )
...
def _h5b_verdict(retention: dict[str, list[float]]) -> dict[str, Any]:
    """H5-B — Jonckheere on retention across (P_min, P_equ, P_max)."""
    groups = [
        retention.get("P_min", []),
        retention.get("P_equ", []),
        retention.get("P_max", []),
...
def _render_md_report(payload: dict[str, Any]) -> str:
    h = payload["verdict"]["h5b_cnn_cifar"]
    lines: list[str] = [
...
def run_pilot(
    *,
    ...
    for arm in ARMS:
        for seed in seeds:
            ...
            run_id = registry.register(
                c_version=C_VERSION,
                profile=f"g4-quinto/step2/{arm}/{c5.combo_id}",
                seed=seed,
    ...
    verdict = {
        "h5b_cnn_cifar": _h5b_verdict(retention),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="G4-quinto Step 2 driver")
    ...
    if not h.get("insufficient_samples"):
        print(
            f"H5-B : monotonic={h['monotonic_observed']} "
