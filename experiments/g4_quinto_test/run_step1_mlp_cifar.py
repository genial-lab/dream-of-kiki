"""G4-quinto Step 1 driver — H5-A test with MLP-on-CIFAR substrate.

Layout : 4 arms x N seeds x 1 HP (C5 anchor) on
``G4HierarchicalCIFARClassifier`` (in_dim = 3072,
hidden = 256-128-64-32). Per-cell pipeline mirrors
``experiments/g4_quater_test/run_step1_deeper`` modulo the
substrate + benchmark swap.

Outputs :
    docs/milestones/g4-quinto-step1-2026-05-03.{json,md}

Pre-reg : docs/osf-prereg-g4-quinto-pilot.md sec 2 (H5-A).
"""
...
from experiments.g4_quinto_test.cifar_mlp_classifier import (  # noqa: E402
...
DEFAULT_OUT_MD = (
    REPO_ROOT / "docs" / "milestones" / "g4-quinto-step1-2026-05-03.md"
)
DEFAULT_REGISTRY_DB = REPO_ROOT / ".run_registry.sqlite"
SMOKE_OUT_JSON = Path("/tmp") / "g4-quinto-step1-smoke.json"
SMOKE_OUT_MD = Path("/tmp") / "g4-quinto-step1-smoke.md"
RETENTION_EPS = 1e-6
BETA_BUFFER_CAPACITY = 256
BETA_BUFFER_FILL_PER_TASK = 32
HIDDEN: tuple[int, int, int, int] = (256, 128, 64, 32)
...
def _dream_episode_deeper(
    clf: G4HierarchicalCIFARClassifier,
    profile: object,
    seed: int,
    *,
    ...
    """Drive one DreamEpisode and mutate MLP-on-CIFAR weights.
    ...
    episode = DreamEpisode(
        ...
        episode_id=f"g4-quinto-step1-{type(profile).__name__}-seed{seed}",
    )
...
def _run_cell(
    ...
    *,
    ...
    # The classifier consumes the flat 3072-dim view from the loader.
    train_dict: dict[str, np.ndarray] = {
        "x_train": tasks[0]["x_train"],
        "y_train": tasks[0]["y_train"],
    }
    clf.train_task(train_dict, epochs=epochs, batch_size=batch_size, lr=lr)
    ...
    for k in range(1, len(tasks)):
        ...
        train_dict = {
            "x_train": tasks[k]["x_train"],
            "y_train": tasks[k]["y_train"],
        }
        clf.train_task(
            train_dict, epochs=epochs, batch_size=batch_size, lr=lr
        )
        _push_task(tasks[k])
...
def _h5a_verdict(retention: dict[str, list[float]]) -> dict[str, Any]:
...
def _render_md_report(payload: dict[str, Any]) -> str:
    h = payload["verdict"]["h5a_mlp_cifar"]
    lines: list[str] = [
    ...
    lines += [
        ...
        (
            ...
            "cifar_mlp_classifier.G4HierarchicalCIFARClassifier`"
        ),
        (
...
def run_pilot(
    *,
    ...
    for arm in ARMS:
        for seed in seeds:
            ...
            run_id = registry.register(
                c_version=C_VERSION,
                profile=f"g4-quinto/step1/{arm}/{c5.combo_id}",
                seed=seed,
    ...
    verdict = {
        "h5a_mlp_cifar": _h5a_verdict(retention),
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="G4-quinto Step 1 driver")
    ...
    if not h.get("insufficient_samples"):
        print(
            f"H5-A : monotonic={h['monotonic_observed']} "
